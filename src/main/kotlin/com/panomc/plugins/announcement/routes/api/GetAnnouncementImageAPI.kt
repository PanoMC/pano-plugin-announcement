...
@Endpoint
class GetAnnouncementImageAPI(
    private val plugin: AnnouncementPlugin,
    private val announcementDao: AnnouncementDao
) : Api() {
    override val paths = listOf(Path("/api/announcements/image/:fileName", RouteType.GET))
    ...
    override suspend fun handle(context: RoutingContext): Result? {
        ...
        if (announcement == null || announcement.imageFileName == null) {
        ...
        // Check if expired
        if (announcement.until != null && announcement.until != 0L && announcement.until < System.currentTimeMillis()) {
            context.response().setStatusCode(404).end()
            return null
        }
        ...
        val etag = "\"${announcement.id}-${announcement.imageFileName}\""
