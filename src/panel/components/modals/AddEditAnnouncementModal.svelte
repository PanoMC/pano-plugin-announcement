  ...
  export function show(newMode, newAnnouncement = null) {
    ...
    if (newAnnouncement) {
      ...
      const state = {
        ...
        showFrom: cloned.showFrom
          ? new Date(cloned.showFrom).toISOString().slice(0, 16)
          : "",
        location: cloned.location || "GLOBAL",
        external: cloned.external || false,
        bannerContents:
          activeType === "BANNER" ? contents : cloned.bannerContents || [""],
        modalContent:
          activeType === "MODAL" ? contents[0] : cloned.modalContent || "",
      };
...
<script>
  ...
  $: isFormValid = (() => {
    ...
    if ($announcement.displayType === "BANNER") {
      ...
      if (
        ...
        $announcement.modalContent.trim() === ""
      )
        return false;
  ...
  function handleFileError(event) {
    ...
    if (error === "INVALID_SIZE") {
      showToast(
        "plugins.pano-plugin-announcement.modals.add-edit.image-error-size",
      );
    } else if (error === "INVALID_TYPE") {
      showToast(
        "plugins.pano-plugin-announcement.modals.add-edit.image-error-type",
      );
    }
  }
      ...
      const data = {
        ...
        effectType:
          $announcement.displayType === "BANNER"
            ? $announcement.bannerEffect
            : "NONE",
        until:
          $announcement.timeType === "TIMED"
            ? new Date($announcement.time).getTime()
            : null,
        showFrom:
  ...
  function removeActiveContent() {
    ...
    announcement.update((a) => {
      a.bannerContents = a.bannerContents.filter(
        (_, i) => i !== activeContentIndex,
      );
      return a;
    });
    ...
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title">
          {$mode === "edit"
            ? $_("modals.add-edit.edit-title")
            : $_("modals.add-edit.new-title")}
        </h5>
        <button
          type="button"
          class="btn-close"
          ...
          <div class="col-md-5">
            <div class="form-check form-switch mb-4">
              <input
                ...
                on:change={(e) =>
                  ($announcement.status = e.target.checked
                    ? "ACTIVE"
                    : "DRAFT")}
              />
            ...
            {#if $announcement.showFromType === "CUSTOM"}
              <div class="form-floating mb-3">
                ...
                />
                <label for="showFrom"
                  >{$_("modals.add-edit.show-from-select")}</label
                >
              </div>
            {/if}
            ...
            {#if $announcement.displayType === "BANNER"}
              <div class="form-floating mb-3">
                ...
                >
                  <option value="GLOBAL"
                    >{$_("modals.add-edit.location-global")}</option
                  >
                  <option value="HOME"
                    >{$_("modals.add-edit.location-home")}</option
                  >
                </select>
              ...
              <div class="form-floating mb-3">
                ...
                >
                  <option value="NONE"
                    >{$_("modals.add-edit.effect-none")}</option
                  >
                  <option value="MARQUEE"
                    >{$_("modals.add-edit.effect-marquee")}</option
                  >
                  <option value="FLASH"
                    >{$_("modals.add-edit.effect-flash")}</option
                  >
                </select>
      ...
      <div class="modal-footer">
        ...
        >
          {$mode === "create"
            ? $_("modals.add-edit.btn-create")
            : $_("modals.add-edit.btn-save")}
        </button>
      </div>
    </div>
  </div>
